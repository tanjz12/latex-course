...
% For output in Chinese
\usepackage{xeCJK,textcomp}
\setCJKmainfont[BoldFont = simhei.ttf, ItalicFont = xjlFont.ttf]{STXIHEI.TTF}
\setCJKmathfont{cmr}
\newfontfamily\myfont{Junicode-Italic.ttf}
...
% -- latex 能自动处理单词，句子和自然段

单词由空格分隔。语段由双换行分割。多余空格，空行不影响输出。

英文双引号需由反引号`和右单引号生成': ``quoted text''.
单引号是这样: `single-quoted text'.
...
- 几重引号: ``\,`
\vskip 2ex
\item 加星号 \emph{star form} 就能显示无编号的方程。
\begin{exampletwouptiny}
\begin{equation*}
F(x) = \int_{a}^{x}{f(t) dt}
\end{equation*}
...
\begin{itemize}
\item \bftt{equation} 和 \bftt{equation*} 被称为情境 \emph{environments}.
\begin{itemize}
  \item 每个情境都在一组\cmdbs{begin} 和 \cmdbs{end}命令间。
  \item \cmd{\$} 标志数学情境的始末。
  \item 有一些命令只在特定情境有效。
  \item 有一些命令的效果随情境改变。
\end{itemize}
...
%  无序列表情境 \\
%\bftt{\$\ldots\$}     & \emph{in-text} 行内式子 \\
%\bftt{\$\$\ldots\$\$} & \emph{displayed} 单行式子 \\
