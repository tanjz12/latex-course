...
\item 改变你编辑文档的思路
\begin{itemize}
\item 非图形界面；文档按模块统一处理 \ldots
\end{itemize}
\end{itemize}

\begin{block}{其他选择？}
\begin{itemize}
  \item \texttt{Word}可处理的图像类型有限；\texttt{.doc, .docx}文档需专门软件读取。
...
\begin{frame}[fragile]{\insertsubsection}
\begin{itemize}
\item \LaTeX{} CJKutf8（已过时）：
\inputminted[frame=single]{latex}{basics-zh-cn.tex}
\item \bftt{CKJutf8}用UTF8编码，\cmd{gbsn}是字体名称。
\item 需要用pdfLaTeX编译
...
\begin{itemize}
\item \LaTeX{} ctex 宏包：
\inputminted[frame=single]{latex}{basics-zh-cn2.tex}
\item 这个宏包接受多种编译方式，但建议使用XeLaTeX或LuaLaTeX。
\item 查看CTAN资源库中的使用手册：
\begin{center}
\fbox{\href{https://ctan.org/pkg/xecjk}{%
点此查看xeCJK}}
\end{center}
\vskip 2em
\begin{center}
\fbox{\href{https://www.ctan.org/pkg/ctex}{%
点此查看ctex}}
\end{center}
\end{itemize}
\end{frame}
...
\item 源代码多余空格不计；LaTeX会根据语言设置决定字距。
\begin{exampletwouptiny}
中英转换时many      spaces及英文
单词间有空格。
中文间         空格不显示。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{\insertsubsection{}：易错点}
\small
\begin{itemize}
\item 英文引号:\\
左上角的 \keystroke{\`{}} 键表左引号；单引号\keystroke{\'{}} 表右引号.
\begin{exampletwouptiny}
单引号: `文本'.
...
\subsection{中文排版}
\begin{frame}[fragile]{\insertsubsection{}: 粗体，斜体和强调}

\begin{itemize}
\item 对于西文（主要指拉丁字母）字体，\textbf{\myfontb bold}和\textit{\myfont italics}经常是单独设计的粗体、花体，而非简单的加粗和倾斜。
\item 伪斜体，伪粗体只是将字符变形，不是单独的字体。
\vskip 1em
\begin{exampletwoup}
{\CJKfontspec[FakeSlant = 0.2]
{STXIHEI.TTF}伪斜体（华文细黑）}

{\CJKfontspec[FakeBold = 2]
{STXIHEI.TTF}伪粗体（华文细黑）}
\end{exampletwoup}
\vskip 1em
\item 若要符合中文排版规范，通常用{\CJKfontspec{AR PL SungtiL GB} 宋体}作基本字体，然后设置\emph{楷体}为强调，\textbf{黑体}为加粗。{\CJKfontspec{STFANGSO.TTF}仿宋}有时用于伪代码。
\vskip 1em
\myfont
\begin{exampletwoup}
\textit{italics表强调}
...
\begin{frame}[fragile]{\insertsubsection{}: xeCJK 宏包}
\begin{itemize}
\item 可编辑基本文档，支持字体、标点调整。
\inputminted[frame=single]{latex}{ex-p1-xecjk.tex}
\end{itemize}
\end{frame}
...
\begin{frame}{\insertsubsection：文档结构}
\begin{itemize}{
\item \LaTeX 里文档结构，包括章节和段落，也可由命令统一控制。
\item article 默认的标题名称都是英文；使用 ctex 则默认将其改为中文。
\item 从大到小有 
\begin{itemize}
\item part，chapter，section，subsection，subsubsection
\item paragraph，subparagraph
\end{itemize} 前五个生成单起一行的标题，后两个是段落。
\begin{itemize}
\item 段首可设特殊格式。
\end{itemize}
...
\begin{frame}[fragile]{\insertsubsection{}: ctex 宏包}
\begin{itemize}
\item 本地编译时，编译方式会对导致略微不同的排版。使用XeLaTeX编译相当于调用XeCJK支持。
\item 日期，章节标题，页眉等默认都已汉化。见下例 \footnote{引自\href{http://www.zwbk.org/MyLemmaShow.aspx?lid=185246}{中文百科}}。

% 这部幻灯使用xeCJK支持，因此ctex命令无法直接实现，不能用woup情境。
% pdf 源码为下页练习。左例代码是 ex-p1-ctex，内容有删减。

   \fbox{\begin{tabular}{l|l}
   \begin{minipage}{0.45\linewidth}
     \inputminted[fontsize=\scriptsize,resetmargins]{latex}{ex-p1-ctex.tex}
   \end{minipage} &
   \begin{minipage}{0.45\linewidth}
     \includegraphics[width=\textwidth,clip,trim=1.5in 3in 1.5in 1in]{ex-p1-ctex.pdf}
   \end{minipage}
   \end{tabular}}
...
\begin{frame}[fragile]{中文排版练习}

\begin{block}{用 \LaTeX 把上张幻灯的 pdf 示例改成这样：}
\begin{center}
\fbox{\href{\fileuri/exercise-part1-ctex-solution.pdf}{%
点此打开文档}}
\end{center}
\end{block}
...
\begin{itemize}
\item 日期改成中文
\item 目录名称改为【目录/Contents】
\item 取消自然段前自动空两格
\item 将小节（section）标题改为【第（阿拉伯数字）节】。
\end{itemize}
\begin{center}
\fbox{\href{\wlnewdoc{exercise-part1-ctex.tex}}{%
点此在 \wllogo{} 中打开该练习的源码}}
\end{center}
\item 试过后，
\fbox{\href{\wlnewdoc{exercise-part1-ctex-solution.tex}}{%
这里有我的代码}}.
\end{itemize}
\end{frame}
